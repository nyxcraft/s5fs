...
static int a_open(int argc, char **argv, FSR *r, const char *usage)
{
	...
	while ((c = getopt(argc, argv, "B:A:d:P:o:sai:")) != -1) {
		...
		case 'A': { s5_endian e = s5_endian_parse(optarg);
		            if (e == S5_NENDIAN) { fprintf(stderr, "%s: bad -A\n", argv[0]); return -1; }
		            forced = (int)e; break; }
		case 'd': dev = optarg; break;
		case 'P': part = optarg[0]; break;
		case 'o': ospec = optarg; break;
		case 's': case 'a': case 'i': break;	/* consumed by the caller below */
		default: fprintf(stderr, "%s\n", usage); return -1;
		}
	}
	if (device_resolve_part(dev, part, ospec, &base, &plen) < 0) return -1;
	if (optind >= argc) { fprintf(stderr, "%s\n", usage); return -1; }
...
/* ================= ncheck ================= */
struct nctx { FSR *r; int sflag; const char *ilist; };

static int wanted_inode(const char *list, uint32_t ino)
{
	const char *p = list; char buf[16];
	while (*p) {
		int n = 0;
		while (*p && *p != ',') { if (n < 15) buf[n++] = *p; p++; }
		buf[n] = '\0';
		if ((uint32_t)strtoul(buf, NULL, 0) == ino) return 1;
		if (*p == ',') p++;
	}
	return 0;
}

static void ncheck_walk(struct nctx *c, uint32_t ino, const char *path)
{
	fsr_inode in;
	uint8_t buf[P11_MAXBSIZE];
	uint32_t nblk, b;
	int show = 1;
	if (fsr_iget(c->r, ino, &in) < 0) return;
	if (c->sflag)
		show = (in.mode & 04000) || (in.mode & 02000) ||
		       (in.mode & P11_IFMT) == P11_IFCHR || (in.mode & P11_IFMT) == P11_IFBLK;
	if (c->ilist) show = wanted_inode(c->ilist, ino);
	if (show && ino != P11_ROOTINO) {
		char m[11]; fs_modestr(in.mode, m);
		printf("%5u  %s  %s\n", ino, m, path);
	}
	if ((in.mode & P11_IFMT) != P11_IFDIR) return;
...
{
	FSR r; struct nctx c; int idx, i;
	c.sflag = 0; c.ilist = NULL;
	for (i = 1; i < argc; i++) {
		if (!strcmp(argv[i], "-s")) c.sflag = 1;
		else if (!strcmp(argv[i], "-i") && i + 1 < argc) c.ilist = argv[++i];
	}
	idx = a_open(argc, argv, &r, "usage: s5fs ncheck [-s] [-i N,N] [-B ..] [-A ..] [-d dev -P part] image");
	if (idx < 0) return 2;
	c.r = &r;
	if (c.sflag) printf("setuid / setgid / device files:\n");
...
/* ================= quot ================= */
struct qrow { int uid; long kb, nf; };
int cmd_quot(int argc, char **argv)
{
	FSR r; int idx; uint32_t ino, nino;
	long *kb, *nf; int u;
	struct qrow *rows;
	int nr = 0;
	...
	if (!kb || !nf || !rows) { fprintf(stderr, "quot: out of memory\n"); return 1; }
	...
	for (ino = 1; ino <= nino; ino++) {
		...
		t = in.mode & P11_IFMT;
		if (t != P11_IFREG && t != P11_IFDIR) continue;
		kb[(uint16_t)in.uid] += ((long)in.size + 1023) / 1024;
		nf[(uint16_t)in.uid]++;
	}
	for (u = 0; u < 65536; u++)
		if (nf[u]) { rows[nr].uid = u; rows[nr].kb = kb[u]; rows[nr].nf = nf[u]; nr++; }
	/* simple insertion sort by kb desc (few distinct uids) */
	{ int a, b; for (a = 1; a < nr; a++) { struct qrow tmp = rows[a]; b = a - 1;
	    while (b >= 0 && rows[b].kb < tmp.kb) { rows[b + 1] = rows[b]; b--; } rows[b + 1] = tmp; } }
	printf("%8s %8s  %s\n", "kbytes", "files", "uid");
	for (u = 0; u < nr; u++)
...
/* ================= du ================= */
static int du_all;
static long du_walk(FSR *r, uint32_t ino, const char *path)
{
	fsr_inode in;
	uint8_t buf[P11_MAXBSIZE];
	uint32_t nblk, b;
...
{
	FSR r; int idx; const char *path = "/"; uint32_t ino; int i;
	du_all = 0;
	for (i = 1; i < argc; i++) if (!strcmp(argv[i], "-a")) du_all = 1;
	idx = a_open(argc, argv, &r, "usage: s5fs du [-a] [-B ..] [-A ..] [-d dev -P part] image [path]");
	if (idx < 0) return 2;
	if (idx + 1 < argc && argv[idx + 1][0] == '/') path = argv[idx + 1];
	ino = fsr_namei(&r, path);
	if (!ino) { fprintf(stderr, "du: %s: not found\n", path); fsr_close(&r); return 1; }
	du_walk(&r, ino, path);
	fsr_close(&r);
	return 0;
}
...
{
	FSR r; int idx; uint8_t sb[P11_MAXBSIZE];
	...
	if (idx < 0) return 2;
	if (fsr_bread(&r, P11_SUPERBLK, sb) < 0) { fprintf(stderr, "df: read error\n"); return 1; }
	...
	for (i = 0; i < P11_NICFREE; i++) fr[i] = (int32_t)r.bo->get32(sb + P11_SB_FREE + 4 * i);
	for (;;) {
		int32_t bno;
		if (nfree <= 0) break;
		bno = fr[--nfree];
		...
		dfree++;
		if (nfree == 0) {
			uint8_t fb[P11_MAXBSIZE]; uint32_t k;
			if (fsr_bread(&r, (uint32_t)bno, fb) < 0) break;
			nfree = (int16_t)r.bo->get16(fb + P11_FB_NFREE);
			for (k = 0; k < P11_NICFREE; k++) fr[k] = (int32_t)r.bo->get32(fb + P11_FB_FREE + 4 * k);
		}
	}
...
{
	FSR r; int idx; uint8_t sb[P11_MAXBSIZE];
	const s5_codec *bo; uint32_t bsz; long long base;
	off_t sboff; int fd, sysv;
	const char *name = NULL, *pack = NULL;

	idx = a_open(argc, argv, &r, "usage: s5fs labelit [-B ..] [-A ..] [-d dev -P part] image [name [pack]]");
	if (idx < 0) return 2;
	bo = r.bo; bsz = r.bsize; base = r.base;
	sboff = (off_t)base + (off_t)P11_SUPERBLK * bsz;
	if (idx + 1 < argc) name = argv[idx + 1];
	if (idx + 2 < argc) pack = argv[idx + 2];
	fsr_close(&r);

	fd = open(argv[idx], name ? O_RDWR : O_RDONLY);
	if (fd < 0) { fprintf(stderr, "labelit: %s: cannot open\n", argv[idx]); return 1; }
	...
	if (!name) {						/* read */
		char b1[16], b2[16];
		if (sysv) {
			memcpy(b1, sb + P11_SB_SVFNAME, 6); b1[6] = '\0';
			memcpy(b2, sb + P11_SB_SVFPACK, 6); b2[6] = '\0';
			printf("fsname '%s'  pack '%s'  (System V labels)\n", b1, b2);
		} else {
			memcpy(b1, sb + P11_SB_FSMNT, 12); b1[12] = '\0';
	...
	/* write */
	if (sysv) {
		memset(sb + P11_SB_SVFNAME, 0, 6); memcpy(sb + P11_SB_SVFNAME, name, strlen(name) > 6 ? 6 : strlen(name));
		if (pack) { memset(sb + P11_SB_SVFPACK, 0, 6); memcpy(sb + P11_SB_SVFPACK, pack, strlen(pack) > 6 ? 6 : strlen(pack)); }
	} else {
		memset(sb + P11_SB_FSMNT, 0, 12); memcpy(sb + P11_SB_FSMNT, name, strlen(name) > 12 ? 12 : strlen(name));
		if (pack) fprintf(stderr, "labelit: note: V7 superblock has one name field; pack ignored\n");
	}
