...
do_sb(void)
{
	...
	if (fsr_bread(&H.r, P11_SUPERBLK, sb) < 0) {
...
do_dir(uint32_t ino)
{
	...
	if ((in.mode & P11_IFMT) != P11_IFDIR) {
		printf("inode %u is not a directory\n", ino);
		return;
	}
	...
	for (b = 0; b < nblk; b++) {
		...
		for (e = 0; e < H.r.ndirect; e++) {
			...
			if ((b * H.r.ndirect + e) * P11_DIRENTSZ >= (uint32_t)in.size)
				break;
			if (di == 0) {
				freecnt++;
				continue;
...
static void
do_block(uint32_t bno)
{
	uint8_t buf[P11_MAXBSIZE];
	uint32_t i, j;
	if (fsr_bread(&H.r, bno, buf) < 0) {
...
do_cat(uint32_t ino)
{
	...
	if ((in.mode & P11_IFMT) == P11_IFDIR) {
		printf("inode %u is a directory\n", ino);
		return;
	}
	while (off < in.size) {
		uint8_t b[8192];
		long want = in.size - off;
		if (want > (long)sizeof b)
			want = sizeof b;
		if (fsr_readfile(&H.r, &in, b, want, off) != want)
			break;
...
static int
lc_cb(void *a, uint32_t ino, const char *name)
{
	struct lc *l = a;
	(void)name;
	if (ino == l->target)
...
static void
do_set(uint32_t ino, const char *field, const char *valstr)
{
	...
	if (!H.writable) {
		printf("read-only; reopen with -w\n");
		return;
	}
	...
	if (!strcmp(field, "mode"))
		H.w.bo->put16(ds + P11_DI_MODE, (uint16_t)val);
	else if (!strcmp(field, "nlink"))
		H.w.bo->put16(ds + P11_DI_NLINK, (uint16_t)val);
	else if (!strcmp(field, "uid"))
		H.w.bo->put16(ds + P11_DI_UID, (uint16_t)val);
	else if (!strcmp(field, "gid"))
		H.w.bo->put16(ds + P11_DI_GID, (uint16_t)val);
	else if (!strcmp(field, "size"))
		H.w.bo->put32(ds + P11_DI_SIZE, (uint32_t)val);
	else {
		printf("unknown field '%s' (mode|nlink|uid|gid|size)\n", field);
		return;
	}
...
do_poke(int argc, char **tok)
{
	...
	if (argc < 4) {
		printf("usage: poke BLK OFF HH [HH...]\n");
		return;
	}
...
cmd_fsdb(int argc, char **argv)
{
	...
	while ((c = getopt(argc, argv, "wB:A:d:P:o:")) != -1) {
		...
		case 'o':
			...
			break;
		default:
			fprintf(stderr, "usage: s5fs fsdb [-w] [-B ..] [-A ..] [-d dev -P part | -o blk] image\n");
			return 2;
		}
	}
	if (optind != argc - 1) {
	...
	for (;;) {
		...
		if (!strcmp(tok[0], "quit") || !strcmp(tok[0], "exit"))
			...
			help();
		else if (!strcmp(tok[0], "sb"))
			do_sb();
		else if ((!strcmp(tok[0], "inode") || !strcmp(tok[0], "i")) && n >= 2)
			do_inode((uint32_t)strtoul(tok[1], NULL, 0));
		else if (!strcmp(tok[0], "dir") && n >= 2)
			do_dir((uint32_t)strtoul(tok[1], NULL, 0));
		else if (!strcmp(tok[0], "map") && n >= 2)
			do_map((uint32_t)strtoul(tok[1], NULL, 0));
		else if ((!strcmp(tok[0], "block") || !strcmp(tok[0], "b")) && n >= 2)
			do_block((uint32_t)strtoul(tok[1], NULL, 0));
		else if (!strcmp(tok[0], "cat") && n >= 2)
			do_cat((uint32_t)strtoul(tok[1], NULL, 0));
		else if (!strcmp(tok[0], "path") && n >= 2) {
			uint32_t x = rw_namei(&H, tok[1]);
			if (x)
				printf("%s -> inode %u\n", tok[1], x);
			else
				printf("%s: not found\n", tok[1]);
		}
		else if (!strcmp(tok[0], "links") && n >= 2)
			do_links((uint32_t)strtoul(tok[1], NULL, 0));
		else if (!strcmp(tok[0], "set") && n >= 4)
			do_set((uint32_t)strtoul(tok[1], NULL, 0), tok[2], tok[3]);
		else if (!strcmp(tok[0], "poke"))
			do_poke(n, tok);
		else
			printf("%s: unknown or missing args (try 'help')\n", tok[0]);
	}
	rw_close(&H);
	return 0;
}
