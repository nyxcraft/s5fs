...
cmd_boot(int argc, char **argv)
{
	...
	if (argc != 3) {
		fprintf(stderr, "usage: s5fs boot image bootfile\n");
		return 2;
	}
	image = argv[1];
	bootf = argv[2];
	...
	if (bf < 0) {
		fprintf(stderr, "s5fs boot: %s: %s\n", bootf, strerror(errno));
		return 1;
	}
	...
	close(bf);
	if (n <= 0) {
		fprintf(stderr, "s5fs boot: %s: empty or unreadable\n", bootf);
		return 1;
	}
	...
	/* skip a 16-byte a.out header if this is a .o rather than raw boot code */
	{
		...
		if (magic == 0407 || magic == 0410 || magic == 0411) {
			off = 16;
			if ((size_t)n <= off) {
				fprintf(stderr, "s5fs boot: bootfile too small\n");
				return 1;
			}
		}
	}
