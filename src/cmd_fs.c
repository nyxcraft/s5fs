...
const char *fs_errstr(int rc) { return strerror(-rc); }
...
static int host_out(const char *dst, unsigned perm, const char *srcbase, char *nb, size_t nbsz)
{
	struct stat st;
...
int fs_copy(RW *h, const char *src, int src_host, const char *dst, int dst_host)
{
	...
	/* host -> image */
	if (src_host && !dst_host) {
		int ifd = open(src, O_RDONLY), rc; struct stat st; char ib[1200];
		const char *target = dst; uint32_t d; fsr_inode di;
		if (ifd < 0 || fstat(ifd, &st) < 0) { if (ifd >= 0) close(ifd); return -errno; }
		d = rw_namei(h, dst);
		if (d && rw_iget(h, d, &di) == 0 && (di.mode & P11_IFMT) == P11_IFDIR) {
			char tmp[1024]; snprintf(tmp, sizeof tmp, "%s", src);
	...
	/* image -> host */
	if (!src_host && dst_host) {
		fsr_inode in; uint32_t ino = rw_namei(h, src); long off = 0; int ofd;
		const char *b = strrchr(src, '/');
		if (!ino || rw_iget(h, ino, &in) < 0) return -ENOENT;
		...
		while (off < in.size) {
			...
			if (fsr_readfile(&h->r, &in, buf, want, off) != want) { close(ofd); return -EIO; }
			if (write(ofd, buf, want) != want) { close(ofd); return -errno; }
			off += want;
		}
		close(ofd);
		return 0;
	...
	/* host -> host */
	{
		int ifd = open(src, O_RDONLY), ofd; struct stat st; ssize_t n;
		const char *b = strrchr(src, '/');
		if (ifd < 0) return -errno;
		if (fstat(ifd, &st) < 0) { close(ifd); return -errno; }
		ofd = host_out(dst, st.st_mode & 0777, b ? b + 1 : src, nb, sizeof nb);
		if (ofd < 0) { close(ifd); return -errno; }
		{ uint8_t buf[65536]; while ((n = read(ifd, buf, sizeof buf)) > 0)
			if (write(ofd, buf, n) != n) { close(ifd); close(ofd); return -errno; } }
		close(ifd); close(ofd);
		return 0;
	}
}

void fs_parse_owner(const char *spec, int *uid, int *gid)
{
	char buf[64], *colon;
	*uid = -1; *gid = -1;
	...
	if (colon) {
		*colon = '\0';
		if (colon[1]) *gid = (int)strtol(colon + 1, NULL, 10);
	}
	if (buf[0]) *uid = (int)strtol(buf, NULL, 10);
}
...
void fs_resolve(const char *cwd, const char *arg, char *out, size_t outsz)
{
	...
	for (t = strtok_r(tmp, "/", &save); t && nc < 256; t = strtok_r(NULL, "/", &save)) {
		...
		if (!strcmp(t, "..")) { if (nc > 0) nc--; continue; }
		comps[nc++] = t;
	}
	if (nc == 0) { snprintf(out, outsz, "/"); return; }
	out[0] = '\0';
...
static int fs_open(int argc, char **argv, RW *h, int writable, const char *extra)
{
	...
	while ((c = getopt(argc, argv, "B:A:d:P:o:")) != -1) {
		...
		case 'A': {
			...
			if (e == S5_NENDIAN) { fprintf(stderr, "%s: bad -A byte order\n", argv[0]); return -1; }
			forced = (int)e; break;
		}
	...
	if (device_resolve_part(dev, part, ospec, &base, &plen) < 0) return -1;
...
struct lsctx { RW *h; int longf; int all; };

static int ls_one(void *arg, uint32_t ino, const char *name)
{
	struct lsctx *c = arg;
	fsr_inode in;
	char m[11], ts[20];

	if (!c->all && name[0] == '.') return 0;
	if (!c->longf) { printf("%s\n", name); return 0; }
	if (rw_iget(c->h, ino, &in) < 0) return 0;
...
{
	RW h; int longf = 0, all = 0, c, i;
	uint32_t bsize = 0, plen = 0; int forced = -1;
	const char *path = "/", *dev = NULL, *ospec = NULL; char part = 0; long long base = 0;
	struct lsctx ctx;
	fsr_inode in;
	uint32_t ino;
	...
	while ((c = getopt(argc, argv, "laB:A:d:P:o:")) != -1) {
		switch (c) {
		case 'l': longf = 1; break;
		case 'a': all = 1; break;
		case 'B': bsize = (uint32_t)strtoul(optarg, NULL, 0); break;
		case 'A': { s5_endian e = s5_endian_parse(optarg);
		            if (e == S5_NENDIAN) { fprintf(stderr, "ls: bad -A\n"); return 2; }
		            forced = (int)e; break; }
		case 'd': dev = optarg; break;
		case 'P': part = optarg[0]; break;
		case 'o': ospec = optarg; break;
	...
	if (optind >= argc) { fprintf(stderr, "usage: s5fs ls [-l] [-a] image [path]\n"); return 2; }
	...
	ctx.h = &h; ctx.longf = longf; ctx.all = all;
...
{
	RW h; int i, first, rc = 0;
	first = fs_open(argc, argv, &h, 0, "path...");
	if (first < 0) return 1;
	if (first >= argc) { fprintf(stderr, "cat: need a path\n"); rw_close(&h); return 2; }
	for (i = first; i < argc; i++) {
		fsr_inode in; long off = 0; uint32_t ino = rw_namei(&h, argv[i]);
		if (!ino || rw_iget(&h, ino, &in) < 0) {
		...
		while (off < in.size) {
			...
			if (fwrite(buf, 1, want, stdout) != (size_t)want) break;
			off += want;
		}
	}
	rw_close(&h);
...
{
	RW h; int first; const char *imgpath, *hostpath; char hbuf[1024];
	fsr_inode in; uint32_t ino; long off = 0; int ofd, rc = 0;

	first = fs_open(argc, argv, &h, 0, "imgpath [hostpath]");
	if (first < 0) return 1;
	if (first >= argc) { fprintf(stderr, "get: need an image path\n"); rw_close(&h); return 2; }
	imgpath = argv[first];
	if (first + 1 < argc) hostpath = argv[first + 1];
	else {
	...
	if (!ino || rw_iget(&h, ino, &in) < 0) {
		fprintf(stderr, "get: %s: no such file\n", imgpath); rw_close(&h); return 1;
	}
	if ((in.mode & P11_IFMT) == P11_IFDIR) {
		fprintf(stderr, "get: %s: is a directory (files only)\n", imgpath); rw_close(&h); return 1;
	}
	if (!strcmp(hostpath, "-")) ofd = STDOUT_FILENO;
	else ofd = open(hostpath, O_WRONLY | O_CREAT | O_TRUNC, in.mode & 0777);
	if (ofd < 0) { fprintf(stderr, "get: %s: %s\n", hostpath, strerror(errno)); rw_close(&h); return 1; }
	while (off < in.size) {
		uint8_t buf[65536];
		long want = in.size - off; if (want > (long)sizeof buf) want = sizeof buf;
		if (fsr_readfile(&h.r, &in, buf, want, off) != want) { rc = 1; break; }
		if (write(ofd, buf, want) != want) { fprintf(stderr, "get: %s: write: %s\n", hostpath, strerror(errno)); rc = 1; break; }
		off += want;
	}
...
{
	RW h; int first, ifd, rc; const char *hostpath, *imgpath; char ibuf[1200];
	struct stat st;

	first = fs_open(argc, argv, &h, 1, "hostpath [imgpath]");
	if (first < 0) return 1;
	if (first >= argc) { fprintf(stderr, "put: need a host path\n"); rw_close(&h); return 2; }
	hostpath = argv[first];
	...
	if (first + 1 < argc) {
		...
		uint32_t d = rw_namei(&h, imgpath); fsr_inode din;
		if (d && rw_iget(&h, d, &din) == 0 && (din.mode & P11_IFMT) == P11_IFDIR) {
			char tmp[1024]; snprintf(tmp, sizeof tmp, "%s", hostpath);
	...
	close(ifd);
	if (rc < 0) { fprintf(stderr, "put: %s: %s\n", imgpath, fs_errstr(rc)); rw_close(&h); return 1; }
...
{
	RW h; uint32_t bsize = 0, plen = 0; int forced = -1, c, rc, sh, dh;
	const char *image, *src, *dst, *dev = NULL, *ospec = NULL; char part = 0; long long base = 0;
	...
	while ((c = getopt(argc, argv, "B:A:d:P:o:")) != -1) {
		...
		case 'B': bsize = (uint32_t)strtoul(optarg, NULL, 0); break;
		case 'A': { s5_endian e = s5_endian_parse(optarg);
		            if (e == S5_NENDIAN) { fprintf(stderr, "cp: bad -A\n"); return 2; }
		            forced = (int)e; break; }
		case 'd': dev = optarg; break;
		case 'P': part = optarg[0]; break;
		case 'o': ospec = optarg; break;
	...
	if (optind + 3 != argc) goto usage;
	image = argv[optind];
	src = argv[optind + 1]; sh = fs_host_path(&src);
	dst = argv[optind + 2]; dh = fs_host_path(&dst);
	if (device_resolve_part(dev, part, ospec, &base, &plen) < 0) return 2;
	...
	if (rc < 0) { fprintf(stderr, "cp: %s\n", fs_errstr(rc)); rw_close(&h); return 1; }
	rw_close(&h);
	return 0;
usage:
	fprintf(stderr,
...
int cmd_mv(int argc, char **argv)
{
	...
	if (first < 0) return 1;
	if (first + 2 != argc) { fprintf(stderr, "usage: s5fs mv image src dst\n"); rw_close(&h); return 2; }
	rc = rw_rename(&h, argv[first], argv[first + 1]);
	if (rc < 0) { fprintf(stderr, "mv: %s -> %s: %s\n", argv[first], argv[first + 1], fs_errstr(rc)); rw_close(&h); return 1; }
	rw_close(&h);
	return 0;
}
...
{
	RW h; int first, i, rc = 0;
	first = fs_open(argc, argv, &h, 1, "path...");
	if (first < 0) return 1;
	if (first >= argc) { fprintf(stderr, "rm: need a path\n"); rw_close(&h); return 2; }
	for (i = first; i < argc; i++) {
		int r = rw_unlink(&h, argv[i]);
		if (r < 0) { fprintf(stderr, "rm: %s: %s\n", argv[i], fs_errstr(r)); rc = 1; }
	}
	rw_close(&h);
	return rc;
}
...
int cmd_rmdir(int argc, char **argv)
{
	...
	if (first < 0) return 1;
	if (first >= argc) { fprintf(stderr, "rmdir: need a directory\n"); rw_close(&h); return 2; }
	for (i = first; i < argc; i++) {
		int r = rw_rmdir(&h, argv[i]);
		if (r < 0) { fprintf(stderr, "rmdir: %s: %s\n", argv[i], fs_errstr(r)); rc = 1; }
	}
	rw_close(&h);
	return rc;
}
...
static int mkdir_p(RW *h, const char *path)
{
	char buf[2048]; size_t i; int rc;
	uint32_t ino = rw_namei(h, path);
	if (ino) return 0;			/* already exists */
	snprintf(buf, sizeof buf, "%s", path);
	for (i = 1; buf[i]; i++) {		/* create each intermediate component */
		if (buf[i] == '/') {
			buf[i] = '\0';
			if (!rw_namei(h, buf)) { rc = rw_mkdir(h, buf, 0755); if (rc < 0 && rc != -EEXIST) return rc; }
			buf[i] = '/';
		}
	}
...
int cmd_mkdir(int argc, char **argv)
{
	...
	while ((c = getopt(argc, argv, "pB:A:d:P:o:")) != -1) {
		...
		case 'B': bsize = (uint32_t)strtoul(optarg, NULL, 0); break;
		case 'A': { s5_endian e = s5_endian_parse(optarg);
		            if (e == S5_NENDIAN) { fprintf(stderr, "mkdir: bad -A\n"); return 2; }
		            forced = (int)e; break; }
		case 'd': dev = optarg; break;
		case 'P': part = optarg[0]; break;
		case 'o': ospec = optarg; break;
		default: fprintf(stderr, "usage: s5fs mkdir [-p] image dir...\n"); return 2;
		}
	}
	if (optind + 1 >= argc) { fprintf(stderr, "usage: s5fs mkdir [-p] image dir...\n"); return 2; }
	if (device_resolve_part(dev, part, ospec, &base, &plen) < 0) return 2;
	...
	for (i = optind + 1; i < argc; i++) {
		...
		if (r < 0) { fprintf(stderr, "mkdir: %s: %s\n", argv[i], fs_errstr(r)); rc = 1; }
	}
	rw_close(&h);
	return rc;
}
...
{
	RW h; int first, i, rc = 0; unsigned perm;
	first = fs_open(argc, argv, &h, 1, "mode path...");
	if (first < 0) return 1;
	if (first + 1 >= argc) { fprintf(stderr, "usage: s5fs chmod image mode path...\n"); rw_close(&h); return 2; }
	perm = (unsigned)strtoul(argv[first], NULL, 8);
	for (i = first + 1; i < argc; i++) {
		int r = rw_chmod(&h, argv[i], perm);
		if (r < 0) { fprintf(stderr, "chmod: %s: %s\n", argv[i], fs_errstr(r)); rc = 1; }
	}
	rw_close(&h);
	return rc;
}
...
{
	RW h; int first, i, rc = 0, uid, gid;
	first = fs_open(argc, argv, &h, 1, "uid[:gid] path...");
	if (first < 0) return 1;
	if (first + 1 >= argc) { fprintf(stderr, "usage: s5fs chown image uid[:gid] path...\n"); rw_close(&h); return 2; }
	fs_parse_owner(argv[first], &uid, &gid);
	for (i = first + 1; i < argc; i++) {
		int r = rw_chown(&h, argv[i], uid, gid);
		if (r < 0) { fprintf(stderr, "chown: %s: %s\n", argv[i], fs_errstr(r)); rc = 1; }
	}
	rw_close(&h);
	return rc;
}

int cmd_chgrp(int argc, char **argv)
{
	RW h; int first, i, rc = 0, gid;
	first = fs_open(argc, argv, &h, 1, "gid path...");
	if (first < 0) return 1;
	if (first + 1 >= argc) { fprintf(stderr, "usage: s5fs chgrp image gid path...\n"); rw_close(&h); return 2; }
	gid = (int)strtol(argv[first], NULL, 10);
	for (i = first + 1; i < argc; i++) {
		int r = rw_chown(&h, argv[i], -1, gid);
		if (r < 0) { fprintf(stderr, "chgrp: %s: %s\n", argv[i], fs_errstr(r)); rc = 1; }
	}
	rw_close(&h);
	return rc;
}
