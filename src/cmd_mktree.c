...
static struct hlink { dev_t dev; ino_t ino; uint32_t s5ino; } *g_hl;
static size_t g_nhl, g_hlcap;

static uint32_t hl_find(dev_t dev, ino_t ino)
{
	size_t i;
	for (i = 0; i < g_nhl; i++)
...
static void hl_add(dev_t dev, ino_t ino, uint32_t s5ino)
{
	if (g_nhl == g_hlcap) {
		g_hlcap = g_hlcap ? g_hlcap * 2 : 64;
		g_hl = realloc(g_hl, g_hlcap * sizeof *g_hl);
		if (!g_hl) { perror("s5fs mktree"); exit(1); }
	}
	g_hl[g_nhl].dev = dev;
	g_hl[g_nhl].ino = ino;
...
static void count_tree(const char *host, uint32_t *n)
{
	DIR *d;
	struct dirent *de;
	char child[4096];
...
static void store_bytes(S5FS *fs, s5fs_inode *in, const uint8_t *data, uint32_t len)
{
...
static void store_fd(S5FS *fs, s5fs_inode *in, int src, uint32_t size)
{
...
static void store_regfile(S5FS *fs, const char *host, uint32_t ino, mode_t hmode)
{
	s5fs_inode in;
	struct stat st;
...
static void write_dir(S5FS *fs, uint32_t ino, uint32_t parent, uint16_t mode,
                      int16_t nlink, const uint16_t *cino, char (*cnm)[16], uint32_t nc)
{
	...
	if (!db) return;
	fs->bo->put16(db + 0 * P11_DIRENTSZ, (uint16_t)ino);   db[2] = '.';
	fs->bo->put16(db + 1 * P11_DIRENTSZ, (uint16_t)parent); db[P11_DIRENTSZ + 2] = '.';
	db[P11_DIRENTSZ + 3] = '.';
	for (i = 0; i < nc; i++) {
...
static void store_devnode(S5FS *fs, uint32_t ino, int isblk, int maj, int min_, unsigned mode)
{
	s5fs_inode in;
	memset(&in, 0, sizeof in);
	in.number = (uint16_t)ino;
...
static void build_lostfound(S5FS *fs, uint32_t ino, uint32_t parent)
{
...
static void build_devdir(S5FS *fs, uint32_t ino, uint32_t parent, const char *spec)
{
	struct dn { uint32_t ino; int isblk, maj, min_; unsigned mode; };
	...
	while (fgets(line, sizeof line, f)) {
		char nm[64], ty; int maj, min_; unsigned mode = 0600, got;
		char *p = line;
		while (*p && isspace((unsigned char)*p)) p++;
		if (*p == '\0' || *p == '#') continue;
		got = sscanf(p, "%63s %c %d %d %o", nm, &ty, &maj, &min_, &mode);
		if (got < 4) { fprintf(stderr, "s5fs mktree: bad /dev line: %s", line); continue; }
		if (nn == cap) {
			cap = cap ? cap * 2 : 32;
			nodes = realloc(nodes, cap * sizeof *nodes);
			cino  = realloc(cino,  cap * sizeof *cino);
			cnm   = realloc(cnm,   cap * sizeof *cnm);
			if (!nodes || !cino || !cnm) { fclose(f); return; }
		}
		nodes[nn].ino = s5fs_ialloc(fs);
		nodes[nn].isblk = (ty == 'b' || ty == 'B');
		nodes[nn].maj = maj; nodes[nn].min_ = min_; nodes[nn].mode = mode;
		cino[nn] = (uint16_t)nodes[nn].ino;
		{ size_t l = strlen(nm); if (l > 15) l = 15; memcpy(cnm[nn], nm, l); cnm[nn][l] = '\0'; }
		nn++;
	}
	fclose(f);
...
static void build_dir(S5FS *fs, const char *host, uint32_t ino, uint32_t parent,
                      const char *devspec, int root)
{
	enum { KID_HOST, KID_LOSTFOUND, KID_DEVDIR, KID_HARDLINK };
	struct kid { uint32_t ino; int kind; int isdir; mode_t hmode; char *host; char nm[16]; };
	...
	while ((de = readdir(d))) {
		...
		if (lstat(child, &st) < 0)
			continue;
		if (S_ISLNK(st.st_mode)) { g_skip_link++; continue; }
		if (!S_ISDIR(st.st_mode) && !S_ISREG(st.st_mode)) { g_skip_spec++; continue; }
		if (nk == cap) {
			cap = cap ? cap * 2 : 16;
			kids = realloc(kids, cap * sizeof *kids);
			if (!kids) { closedir(d); return; }
		}
	...
	if (root) {				/* synthesize lost+found (+ /dev) */
		...
		for (j = 0; j < add; j++) {
			...
			kids[nk].isdir = 1;
			kids[nk].host = NULL;
			if (j == 0) { kids[nk].kind = KID_LOSTFOUND; strcpy(kids[nk].nm, "lost+found"); }
			else        { kids[nk].kind = KID_DEVDIR;    strcpy(kids[nk].nm, "dev"); }
			nsub++; nk++;
		}
	}
	...
	if (!cino || !cnm) { free(cino); free(cnm); free(kids); return; }
	for (i = 0; i < nk; i++) {
		cino[i] = (uint16_t)kids[i].ino;
		strncpy(cnm[i], kids[i].nm, 15); cnm[i][15] = '\0';
	}
	if (stat(host, &st) < 0)
		st.st_mode = 0755;
	...
	for (i = 0; i < nk; i++) {
		...
		if (kids[i].kind == KID_HARDLINK)
			{ s5fs_bumplink(fs, kids[i].ino); g_links++; }
...
int cmd_mktree(int argc, char **argv)
{
	s5fs_opts opts;
	S5FS fs;
	...
	while ((c = getopt(argc, argv, "B:a:d:b:s:t:D:P:o:")) != -1) {
		...
		case 'D': devspec    = optarg;                                  break;
		case 'P': part = optarg[0];                                     break;
		case 'o': ospec = optarg;                                       break;
		case 'a':
			opts.endian = s5_endian_parse(optarg);
			if (opts.endian == S5_NENDIAN) { fprintf(stderr, "s5fs mktree: bad -a\n"); return 2; }
			break;
		case 'd':
			dev = device_find(optarg);
			if (!dev) { fprintf(stderr, "s5fs mktree: unknown device '%s'\n", optarg); return 2; }
			break;
		default: usage();
		}
	}
	...
	if (sectors) { if (blocks) { fprintf(stderr, "s5fs mktree: give -b or -s, not both\n"); return 2; } blocks = sectors / per; }
	if (device_resolve_part(dev ? dev->name : NULL, part, ospec, &base, &plen) < 0) return 2;
	if (base != 0 || plen != 0) {
		if (plen == 0) { fprintf(stderr, "s5fs mktree: partition needs a length (use -d -P, or -o START:LEN)\n"); return 2; }
		opts.base = base;
		blocks = plen / per;
	}
	if (blocks == 0) { fprintf(stderr, "s5fs mktree: need a size (-d, -b, or -s)\n"); usage(); }
	...
	{ uint32_t n = 0; count_tree(rootdir, &n); ninode = n + devn + 16; }
	opts.ninode = ninode;
