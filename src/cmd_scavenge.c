...
static void
mark_ind(uint32_t bno, int level)
{
	uint8_t buf[P11_MAXBSIZE];
	uint32_t i;
	if (bno == 0 || bno >= R.fsize)
		return;
	used[bno] = 1;
	if (level == 0 || fsr_bread(&R, bno, buf) < 0)
		return;
...
build_used(void)
{
	...
	if (!used) {
		fprintf(stderr, "s5fs scavenge: out of memory\n");
		exit(1);
	}
	for (ino = 1; ino <= nino; ino++) {
		...
		if ((in.mode & P11_IFMT) != P11_IFREG && (in.mode & P11_IFMT) != P11_IFDIR)
			continue;
		for (i = 0; i < R.laddr; i++)
			if (in.addr[i] > 0 && (uint32_t)in.addr[i] < R.fsize)
				used[in.addr[i]] = 1;
...
static int
printable_name(const uint8_t *p)
{
	int i;
	if (p[0] == 0)
		return 0;
...
deleted_names(void)
{
	...
	for (ino = 1; ino <= nino; ino++) {
		...
		for (b = 0; b < nblk; b++) {
			...
			for (e = 0; e * P11_DIRENTSZ < (uint32_t)got; e++) {
				...
				if (R.bo->get16(d) != 0)
					continue; /* live entry */
				if (!printable_name(d + 2))
					continue; /* never-used slot */
				memcpy(nm, d + 2, P11_DIRSIZ);
				nm[P11_DIRSIZ] = '\0';
				if (found == 0)
					printf("Deleted names (inode number cleared, name survived):\n");
	...
	if (!found)
		printf("Deleted names: none found.\n");
	return found;
}

/* ---- classify an unreferenced block by its leading bytes ---- */
static int
classify(const uint8_t *b)
{
	uint32_t i, pr = 0, zero = 0;
	uint16_t w0 = R.bo->get16(b);
	if (w0 == 0407 || w0 == 0410 || w0 == 0411)
		return SIG_AOUT;
	if (w0 == 0177545)
		return SIG_AR; /* PDP-11 ar magic */
	if (R.bsize >= 512 && memcmp(b + 257, "ustar", 5) == 0)
		return SIG_TAR;
	...
	}
	{
		...
		for (i = 0; i < R.bsize; i++) {
			if (b[i] == 0)
				zero++;
			else if ((b[i] >= 32 && b[i] < 127) || b[i] == 9 || b[i] == 10 || b[i] == 13)
				pr++;
			else
				np++;
		}
...
static long
carve(uint32_t bno, long len, const char *outdir, const char *base)
{
	char path[1200];
	...
	f = fopen(path, "wb");
	if (!f) {
		fprintf(stderr, "s5fs scavenge: %s: cannot create\n", path);
		return -1;
	}
	while (done < len) {
		uint32_t cur = bno + (uint32_t)(done / R.bsize);
		long want = len - done;
		if (want > (long)R.bsize)
			want = R.bsize;
		if (cur >= R.fsize || fsr_bread(&R, cur, b) < 0)
			break;
		if (fwrite(b, 1, want, f) != (size_t)want)
			break;
		done += want;
	}
	fclose(f);
...
cmd_scavenge(int argc, char **argv)
{
	...
	while ((c = getopt(argc, argv, "B:A:d:P:o:x:")) != -1) {
		...
		case 'x':
			...
			break;
		default:
			fprintf(stderr, "usage: s5fs scavenge [-B ..] [-A ..] [-d dev -P part | -o blk] [-x DIR] image\n");
			return 2;
		}
	}
	if (device_resolve_part(dev, part, ospec, &base, &plen) < 0)
		return 2;
	if (optind != argc - 1) {
		fprintf(stderr, "usage: s5fs scavenge [...] [-x DIR] image\n");
		return 2;
	}
	...
	for (bno = R.isize; bno < R.fsize; bno++) {
		...
		if (sig == SIG_AOUT) {
			...
			if (outdir)
				carve(bno, (long)(R.fsize - bno) * R.bsize > 65536 ? 65536 : 1024, outdir, "tar");
		}
	}
	printf("  totals:");
	for (i = 0; i < SIG_N; i++)
		if (tally[i])
			printf("  %s=%u", signame[i], tally[i]);
	printf("\n");
	...
	if (outdir) {
		...
		for (bno = R.isize; bno < R.fsize; bno++) {
			...
			if (fsr_bread(&R, bno, b) < 0)
				continue;
			if (classify(b) == SIG_TEXT) {
				carve(bno, R.bsize, outdir, "text");
				if (++nt >= 200) {
					printf("  (stopping at 200 text blocks)\n");
					break;
				}
			}
		}
