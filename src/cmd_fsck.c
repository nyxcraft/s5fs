...
static void die(const char *m) { fprintf(stderr, "s5fs fsck: %s\n", m); exit(1); }

static void rdblk(CK *c, uint32_t bno, uint8_t *buf)
{
...
static void wtblk(CK *c, uint32_t bno, const uint8_t *buf)
{
	off_t off = (off_t)(c->base + (int64_t)bno * c->bsize);
	if (lseek(c->fd, off, SEEK_SET) < 0 ||
...
static void rep_bfree(CK *c, int32_t *fr, int *nf, int32_t *tf, int32_t bno)
{
	if (bno != 0)
		(*tf)++;
	if (*nf >= P11_NICFREE) {
...
static void mark(CK *c, uint32_t bno, const char *what)
{
	if (bno == 0)
		return;
...
static void mark_indirect(CK *c, uint32_t bno, int level)
{
	uint8_t *buf;
	uint32_t i;
	...
	if (bno == 0)
		return;
	mark(c, bno, level == 1 ? "single-indirect" :
	            level == 2 ? "double-indirect" : "triple-indirect");
	if (bno < c->isize || bno >= c->fsize)
		return;
	buf = malloc(c->bsize);
...
static int read_inode(CK *c, uint32_t ino, uint16_t *mode, uint16_t *nlink,
...
static uint32_t bmap(CK *c, const int32_t *addr, uint32_t lbn)
{
	uint8_t *buf;
	...
	if (lbn < per) {				/* single indirect */
		...
		if (phys) { rdblk(c, phys, buf); phys = c->bo->get32(buf + 4 * lbn); }
	} else if (lbn < per * per) {			/* double indirect */
		lbn -= per;
		phys = (uint32_t)addr[c->laddr + 1];
		if (phys) { rdblk(c, phys, buf); phys = c->bo->get32(buf + 4 * (lbn / per)); }
		if (phys) { rdblk(c, phys, buf); phys = c->bo->get32(buf + 4 * (lbn % per)); }
	} else {					/* triple indirect */
		lbn -= per * per;
		phys = (uint32_t)addr[c->laddr + 2];
		if (phys) { rdblk(c, phys, buf); phys = c->bo->get32(buf + 4 * (lbn / (per * per))); }
		if (phys) { rdblk(c, phys, buf); phys = c->bo->get32(buf + 4 * ((lbn / per) % per)); }
		if (phys) { rdblk(c, phys, buf); phys = c->bo->get32(buf + 4 * (lbn % per)); }
	}
	free(buf);
...
static void walk_dir(CK *c, uint32_t ino, const char *path)
{
	uint16_t mode, nlink;
...
static int fsck_run(const char *path, uint32_t bsize, s5_endian forced,
                    int checks, int list, int repair, int64_t base)
{
	...
	if (!c.use || !dnlink || !linkcnt || !isdir || !alloc) die("out of memory");

	/* 1) every allocated inode's block tree; note link counts + dir flags */
	for (ino = 1; ino <= nino; ino++) {
		uint16_t mode, nlink;
		int32_t size, addr[P11_MAXNADDR];
		if (read_inode(&c, ino, &mode, &nlink, &size, addr) < 0) break;
		if (mode == 0) continue;
		files++;
		alloc[ino] = 1;
		dnlink[ino] = nlink;
	...
	if (checks & CHK_BLOCKS) {
		...
		for (;;) {
			int32_t bno;
			if (nfree <= 0) { fprintf(stderr, "  ! free list count underflow\n"); c.errors++; break; }
			bno = free_[--nfree];
			if (bno == 0) break;			/* list terminator */
			if ((uint32_t)bno < c.isize || (uint32_t)bno >= c.fsize) {
				fprintf(stderr, "  ! free block %d out of range\n", bno); c.errors++; continue;
			}
			if (c.use[bno] & 0x7f) { fprintf(stderr, "  ! block %d free AND used\n", bno); c.errors++; }
			c.use[bno] |= 0x80;
	...
	if (checks & CHK_BLOCKS) {
		...
		if (missing) { fprintf(stderr, "  ! %u data blocks neither used nor free\n", missing); c.errors++; }
		if (both)    { fprintf(stderr, "  ! %u data blocks both used and free\n", both); c.errors++; }
	...
	if (checks & CHK_LINKS) {
		...
		free(db);
		for (ino = 1; ino <= nino; ino++) {
			if (linkcnt[ino] == dnlink[ino]) continue;
			bad++;
			if (dnlink[ino] == 0)
...
int cmd_fsck(int argc, char **argv)
{
	...
	while ((opt = getopt(argc, argv, "B:A:lpd:P:o:")) != -1) {
		...
		case 'l': list = 1; break;
		case 'p': repair = 1; break;
		case 'A':
			forced = s5_endian_parse(optarg);
			if (forced == S5_NENDIAN) die("bad -A byte order (use pdp11|le|be)");
			break;
		case 'd': dev = optarg; break;
		case 'P': part = optarg[0]; break;
		case 'o': ospec = optarg; break;
		default: fprintf(stderr, "usage: s5fs fsck [-B 512|1024] [-A pdp11|le|be] [-d dev -P part | -o blk] [-l] [-p] image\n"); return 2;
		}
	}
	if (optind != argc - 1) { fprintf(stderr, "usage: s5fs fsck [-B 512|1024] [-A pdp11|le|be] [-d dev -P part | -o blk] [-l] [-p] image\n"); return 2; }
...
int cmd_icheck(int argc, char **argv)		/* block/free-list check (-s salvages) */
{
	uint32_t bsize = 1024, plen = 0;
	s5_endian forced = S5_NENDIAN;
	int repair = 0, opt;
	...
	while ((opt = getopt(argc, argv, "B:A:sd:P:o:")) != -1) {
		...
		case 'P': part = optarg[0]; break;
		case 'o': ospec = optarg; break;
		default: fprintf(stderr, "usage: s5fs icheck [-B 512|1024] [-A pdp11|le|be] [-d dev -P part | -o blk] [-s] image\n"); return 2;
		}
	}
	if (optind != argc - 1) { fprintf(stderr, "usage: s5fs icheck [-B 512|1024] [-A pdp11|le|be] [-d dev -P part | -o blk] [-s] image\n"); return 2; }
	if (device_resolve_part(dev, part, ospec, &base, &plen) < 0) return 2;
...
int cmd_dcheck(int argc, char **argv)		/* directory link-count check */
{
	...
	while ((opt = getopt(argc, argv, "B:A:d:P:o:")) != -1) {
		...
		case 'P': part = optarg[0]; break;
		case 'o': ospec = optarg; break;
		default: fprintf(stderr, "usage: s5fs dcheck [-B 512|1024] [-A pdp11|le|be] [-d dev -P part | -o blk] image\n"); return 2;
		}
	}
	if (optind != argc - 1) { fprintf(stderr, "usage: s5fs dcheck [-B 512|1024] [-A pdp11|le|be] [-d dev -P part | -o blk] image\n"); return 2; }
	if (device_resolve_part(dev, part, ospec, &base, &plen) < 0) return 2;
	return fsck_run(argv[optind], bsize, forced, CHK_LINKS, 0, 0, base);
}

int cmd_clri(int argc, char **argv)		/* clear (zero) an inode by number */
{
	CK c;
	uint8_t sb[P11_MAXBSIZE];
	uint32_t bsize = 1024, plen = 0;
	...
	while ((opt = getopt(argc, argv, "B:A:d:P:o:")) != -1) {
		...
		case 'P': part = optarg[0]; break;
		case 'o': ospec = optarg; break;
		default: fprintf(stderr, "usage: s5fs clri [-B 512|1024] [-A pdp11|le|be] [-d dev -P part | -o blk] image inode...\n"); return 2;
		}
	}
	...
	if (c.fd < 0) { fprintf(stderr, "s5fs clri: %s: %s\n", argv[optind], strerror(errno)); return 1; }
	c.bsize = bsize; c.inopb = bsize / P11_DINODESZ;
	c.naddr = (bsize == 1024) ? 7 : 13; c.laddr = c.naddr - 3;
	c.nindir = bsize / 4; c.ndirect = bsize / P11_DIRENTSZ;
	rdblk(&c, P11_SUPERBLK, sb);
	c.bo = (forced != S5_NENDIAN) ? s5_codec_for(forced) : detect_bo(c.fd, bsize, sb);
	if (!c.bo) die("cannot determine byte order (try -A/-B)");
	c.isize = c.bo->get16(sb + P11_SB_ISIZE);
	c.fsize = c.bo->get32(sb + P11_SB_FSIZE);
	for (i = optind + 1; i < argc; i++) {
		...
		if (inum == 0 || blk >= c.isize) { fprintf(stderr, "s5fs clri: inode %u out of range\n", inum); continue; }
		rdblk(&c, blk, b);
