...
void rw_close(RW *h)
{
	if (h->writable) { s5fs_finish(&h->w); close(h->w.fd); }
	fsr_close(&h->r);
	h->writable = 0;
}

uint32_t rw_namei(RW *h, const char *path)      { return fsr_namei(&h->r, path); }
int      rw_iget (RW *h, uint32_t ino, fsr_inode *o) { return fsr_iget(&h->r, ino, o); }
...
static uint32_t per_at(RW *h, int levels)
{
	uint32_t p = 1;
	int k;
	for (k = 1; k < levels; k++)
...
static uint32_t ind_map(RW *h, uint32_t iblk, uint32_t idx, int levels, int alloc)
{
...
uint32_t rw_bmap(RW *h, uint8_t *ds, uint32_t lbn, int alloc, int *dirty)
{
	...
	lbn -= h->w.laddr;
	if (lbn < per)                 { lvl = 1; slot = h->w.laddr;     idx = lbn; }
	else if (lbn < per * per)      { lbn -= per;       lvl = 2; slot = h->w.laddr + 1; idx = lbn; }
	else if (lbn < per * per * per){ lbn -= per * per; lvl = 3; slot = h->w.laddr + 2; idx = lbn; }
	else return 0;
...
static void bfree_at(RW *h, uint8_t *ds, uint32_t lbn)
{
	...
	if (lbn < h->w.laddr) {
		...
		if (b) { s5fs_bfree(&h->w, (int32_t)b); h->w.bo->put24(ds + P11_DI_ADDR + 3 * lbn, 0); }
		return;
	}
	lbn -= h->w.laddr;
	if (lbn < per)                 { lvl = 1; slot = h->w.laddr;     idx = lbn; }
	...
	if (b) { s5fs_bfree(&h->w, (int32_t)b); h->w.bo->put32(ib + 4 * idx, 0); s5fs_wtblk(&h->w, iblk, ib); }
}

/* parent directory inode of `path`, with the leaf name copied to `leaf` */
static uint32_t parent_of(RW *h, const char *path, char *leaf, size_t leafsz)
{
	char buf[2048], *slash;
	...
	leaf[leafsz - 1] = '\0';
	if (slash == buf) return fsr_namei(&h->r, "/");
	*slash = '\0';
	return fsr_namei(&h->r, buf);
}

static uint32_t ialloc_scan(RW *h)
{
...
static void write_new_inode(RW *h, uint32_t ino, uint16_t mode, uint32_t addr0,
                            int16_t nlink, int32_t size)
{
	...
	if (addr0) h->w.bo->put24(ds + P11_DI_ADDR, addr0);
...
static int dir_add(RW *h, uint32_t pino, const char *name, uint32_t cino)
{
	...
	for (b = 0; b < nblk; b++) {
		...
		for (e = 0; e < h->w.ndirect; e++) {
			if ((b * h->w.ndirect + e) * P11_DIRENTSZ >= psize) break;
...
static uint32_t dir_remove(RW *h, uint32_t pino, const char *name)
{
	uint8_t pb[P11_MAXBSIZE], db[P11_MAXBSIZE];
...
static void ind_free(RW *h, uint32_t iblk, int levels)
{
	uint8_t ib[P11_MAXBSIZE];
	uint32_t i;
	if (!iblk) return;
	...
	for (i = 0; i < h->w.nindir; i++) {
		...
		if (!b) continue;
		if (levels == 1) s5fs_bfree(&h->w, (int32_t)b);
		else ind_free(h, b, levels - 1);
	}
	s5fs_bfree(&h->w, (int32_t)iblk);
}

static void ifree_blocks(RW *h, uint8_t *ds)
{
	uint32_t i;
	for (i = 0; i < h->w.laddr; i++) {
		uint32_t b = h->w.bo->get24(ds + P11_DI_ADDR + 3 * i);
		if (b) s5fs_bfree(&h->w, (int32_t)b);
	}
...
static int nonempty_cb(void *arg, uint32_t ino, const char *name)
{
	(void)ino;
	if (strcmp(name, ".") && strcmp(name, "..")) { *(int *)arg = 1; return 1; }
	return 0;
}

static void bump_nlink(RW *h, uint32_t ino, int delta)
{
	uint8_t pb[P11_MAXBSIZE]; uint32_t blk, off; uint8_t *ds;
	rw_ino_loc(h, ino, &blk, &off); s5fs_rdblk(&h->w, blk, pb); ds = pb + off;
	h->w.bo->put16(ds + P11_DI_NLINK, (uint16_t)(h->w.bo->get16(ds + P11_DI_NLINK) + delta));
	s5fs_wtblk(&h->w, blk, pb);
}

static void set_dotdot(RW *h, uint32_t dino, uint32_t newparent)
{
	uint8_t pb[P11_MAXBSIZE], db[P11_MAXBSIZE]; uint32_t blk, off, phys; uint8_t *ds; int d = 0;
	rw_ino_loc(h, dino, &blk, &off); s5fs_rdblk(&h->w, blk, pb); ds = pb + off;
...
static void unref_inode(RW *h, uint32_t ino, int is_dir, uint32_t parent)
{
	uint8_t pb[P11_MAXBSIZE]; uint32_t blk, off; uint8_t *ds; int16_t nl;
	rw_ino_loc(h, ino, &blk, &off); s5fs_rdblk(&h->w, blk, pb); ds = pb + off;
	if (is_dir) {
		ifree_blocks(h, ds); memset(ds, 0, P11_DINODESZ); h->w.s_tinode++;
		bump_nlink(h, parent, -1);
	} else {
		nl = (int16_t)h->w.bo->get16(ds + P11_DI_NLINK);
		if (--nl <= 0) { ifree_blocks(h, ds); memset(ds, 0, P11_DINODESZ); h->w.s_tinode++; }
		else h->w.bo->put16(ds + P11_DI_NLINK, (uint16_t)nl);
	}
	s5fs_wtblk(&h->w, blk, pb);
}
...
{
	char leaf[256]; uint32_t pino, cino;
	if (!h->writable) return -EROFS;
	pino = parent_of(h, path, leaf, sizeof leaf); if (!pino) return -ENOENT;
	if (fsr_namei(&h->r, path)) return -EEXIST;
	cino = ialloc_scan(h); if (!cino) return -ENOSPC;
	write_new_inode(h, cino, (uint16_t)(P11_IFREG | (perm & 07777)), 0, 1, 0);
	if (dir_add(h, pino, leaf, cino) < 0) return -ENOSPC;
	if (out_ino) *out_ino = cino;
	rw_sync(h);
	return 0;
}
...
{
	char leaf[256]; uint32_t pino, cino, dblk;
	...
	h->w.bo->put16(db + 0 * P11_DIRENTSZ, (uint16_t)cino); db[2] = '.';
...
{
	char leaf[256]; uint32_t pino, cino, blk, o; uint8_t pb[P11_MAXBSIZE]; uint8_t *ds;
	fsr_inode in; int16_t nl;
	if (!h->writable) return -EROFS;
	pino = parent_of(h, path, leaf, sizeof leaf); if (!pino) return -ENOENT;
	cino = fsr_namei(&h->r, path); if (!cino) return -ENOENT;
	if (fsr_iget(&h->r, cino, &in) == 0 && (in.mode & P11_IFMT) == P11_IFDIR) return -EISDIR;
	if (dir_remove(h, pino, leaf) == 0) return -ENOENT;
	rw_ino_loc(h, cino, &blk, &o); s5fs_rdblk(&h->w, blk, pb); ds = pb + o;
	nl = (int16_t)h->w.bo->get16(ds + P11_DI_NLINK);
	if (--nl <= 0) {
		ifree_blocks(h, ds);
		memset(ds, 0, P11_DINODESZ);
...
{
	char leaf[256]; uint32_t pino, cino, blk, o; uint8_t pb[P11_MAXBSIZE]; uint8_t *ds;
	fsr_inode in; int ne = 0;
	if (!h->writable) return -EROFS;
	pino = parent_of(h, path, leaf, sizeof leaf); if (!pino) return -ENOENT;
	cino = fsr_namei(&h->r, path); if (!cino) return -ENOENT;
	if (fsr_iget(&h->r, cino, &in) < 0) return -ENOENT;
	if ((in.mode & P11_IFMT) != P11_IFDIR) return -ENOTDIR;
	fsr_readdir(&h->r, &in, nonempty_cb, &ne);
	if (ne) return -ENOTEMPTY;
	if (dir_remove(h, pino, leaf) == 0) return -ENOENT;
	rw_ino_loc(h, cino, &blk, &o); s5fs_rdblk(&h->w, blk, pb); ds = pb + o;
	ifree_blocks(h, ds);
	memset(ds, 0, P11_DINODESZ);
...
int rw_rename(RW *h, const char *from, const char *to)
{
	...
	if (!h->writable) return -EROFS;
	fpino = parent_of(h, from, fleaf, sizeof fleaf); if (!fpino) return -ENOENT;
	cino  = fsr_namei(&h->r, from);                  if (!cino)  return -ENOENT;
	tpino = parent_of(h, to, tleaf, sizeof tleaf);   if (!tpino) return -ENOENT;
	if (fsr_iget(&h->r, cino, &cin) < 0) return -ENOENT;
	...
	if (tino) {
		fsr_inode tin;
		int tdir;
		if (tino == cino) return 0;
		if (fsr_iget(&h->r, tino, &tin) < 0) return -ENOENT;
		tdir = (tin.mode & P11_IFMT) == P11_IFDIR;
		if (tdir != sdir) return tdir ? -EISDIR : -ENOTDIR;
		if (tdir) { int ne = 0; fsr_readdir(&h->r, &tin, nonempty_cb, &ne); if (ne) return -ENOTEMPTY; }
		dir_remove(h, tpino, tleaf);
		unref_inode(h, tino, tdir, tpino);
	}
	if (dir_add(h, tpino, tleaf, cino) < 0) return -ENOSPC;
...
{
	uint8_t pb[P11_MAXBSIZE]; uint32_t ino, blk, off; uint8_t *ds; uint16_t m;
	if (!h->writable) return -EROFS;
	ino = fsr_namei(&h->r, path); if (!ino) return -ENOENT;
	rw_ino_loc(h, ino, &blk, &off); s5fs_rdblk(&h->w, blk, pb); ds = pb + off;
...
{
	uint8_t pb[P11_MAXBSIZE]; uint32_t ino, blk, off; uint8_t *ds;
	if (!h->writable) return -EROFS;
	ino = fsr_namei(&h->r, path); if (!ino) return -ENOENT;
	rw_ino_loc(h, ino, &blk, &off); s5fs_rdblk(&h->w, blk, pb); ds = pb + off;
	if (uid >= 0) h->w.bo->put16(ds + P11_DI_UID, (uint16_t)uid);
	if (gid >= 0) h->w.bo->put16(ds + P11_DI_GID, (uint16_t)gid);
	s5fs_wtblk(&h->w, blk, pb);
	rw_sync(h);
	return 0;
}
...
long rw_pwrite(RW *h, uint32_t ino, const void *vbuf, long size, long off)
{
	const uint8_t *buf = vbuf;
	uint8_t pb[P11_MAXBSIZE], dblk[P11_MAXBSIZE]; uint32_t blk, o; uint8_t *ds;
	uint16_t mode; int32_t fsize; long done = 0; int dirty = 0;

	if (!h->writable) return -EROFS;
	rw_ino_loc(h, ino, &blk, &o); s5fs_rdblk(&h->w, blk, pb); ds = pb + o;
	mode = h->w.bo->get16(ds + P11_DI_MODE);
	if ((mode & P11_IFMT) == P11_IFDIR) return -EISDIR;
	...
	if (done == 0 && size > 0) return -EFBIG;
	if (off + done > fsize) { h->w.bo->put32(ds + P11_DI_SIZE, (uint32_t)(off + done)); dirty = 1; }
	if (dirty) s5fs_wtblk(&h->w, blk, pb);
	return done;
}

int rw_truncate(RW *h, uint32_t ino, long len)
{
	uint8_t pb[P11_MAXBSIZE]; uint32_t blk, o; uint8_t *ds; uint16_t mode; int32_t fsize;
	if (!h->writable) return -EROFS;
	rw_ino_loc(h, ino, &blk, &o); s5fs_rdblk(&h->w, blk, pb); ds = pb + o;
	mode = h->w.bo->get16(ds + P11_DI_MODE);
	if ((mode & P11_IFMT) == P11_IFDIR) return -EISDIR;
...
static void set_meta(RW *h, uint32_t ino, unsigned perm, int32_t atime, int32_t mtime)
{
	uint8_t pb[P11_MAXBSIZE]; uint32_t blk, off; uint8_t *ds; uint16_t m;
	rw_ino_loc(h, ino, &blk, &off); s5fs_rdblk(&h->w, blk, pb); ds = pb + off;
	m = h->w.bo->get16(ds + P11_DI_MODE);
	h->w.bo->put16(ds + P11_DI_MODE, (uint16_t)((m & P11_IFMT) | (perm & 07777)));
	h->w.bo->put32(ds + P11_DI_ATIME, (uint32_t)atime);
...
int rw_put_fd(RW *h, const char *path, int srcfd, unsigned perm, int32_t mtime)
{
	...
	if (ino) {
		if (fsr_iget(&h->r, ino, &in) < 0) return -EIO;
		if ((in.mode & P11_IFMT) == P11_IFDIR) return -EISDIR;
		if ((in.mode & P11_IFMT) != P11_IFREG) return -EPERM;
