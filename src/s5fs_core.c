...
static void
s5fs_fail(S5FS *fs, const char *msg)
{
...
static uint32_t
s5fs_itod(const S5FS *fs, uint32_t ino)
{
	return (ino + 2 * fs->inopb - 1) / fs->inopb;
}

static uint32_t
s5fs_itoo(const S5FS *fs, uint32_t ino)
{
	return (ino + 2 * fs->inopb - 1) % fs->inopb;
}
...
void
s5fs_rdblk(S5FS *fs, uint32_t bno, uint8_t *buf)
{
...
void
s5fs_wtblk(S5FS *fs, uint32_t bno, const uint8_t *buf)
{
	off_t off = (off_t)(fs->base + (int64_t)bno * fs->bsize);
...
static void
s5fs_put_fblk(S5FS *fs, int32_t bno)
{
...
static void
s5fs_get_fblk(S5FS *fs, int32_t bno)
{
	uint8_t blk[P11_MAXBSIZE];
	int i;
...
void
s5fs_bfree(S5FS *fs, int32_t bno)
{
	if (bno != 0)
		fs->s_tfree++;
...
void
s5fs_newblk(S5FS *fs, int *dbc, uint8_t *db, int *ibc, int32_t *ib)
{
...
void
s5fs_entry(S5FS *fs, int *dbc, uint8_t *db, int *ibc, int32_t *ib,
...
void
s5fs_iput(S5FS *fs, s5fs_inode *in, int ibc, int32_t *ib)
{
...
static int32_t
s5fs_build_ind(S5FS *fs, const int32_t *da, uint32_t count, int level)
{
	uint8_t *blk;
	int32_t bno;
	uint32_t per = 1, e;
...
int
s5fs_setblocks(S5FS *fs, s5fs_inode *in, const int32_t *da, uint32_t n)
{
	...
	rest -= c;
	if (rest) {
		s5fs_fail(fs, "file too large for filesystem");
		return -1;
	}
	return 0;
}
...
void
s5fs_writeinode(S5FS *fs, const s5fs_inode *in)
{
	uint8_t buf[P11_MAXBSIZE];
	uint32_t d, off, i;
	uint8_t *dp;
...
void
s5fs_bumplink(S5FS *fs, uint32_t ino)
{
	uint8_t buf[P11_MAXBSIZE];
	uint32_t d;
	uint8_t *dp;
...
static void
s5fs_mklost(S5FS *fs, s5fs_inode *par)
{
	s5fs_inode in;
...
int
s5fs_begin(S5FS *fs, int fd, uint32_t nblocks, const s5fs_opts *opts)
{
	s5fs_opts def = {0};
...
int
s5fs_mount(S5FS *fs, int fd, uint32_t bsize, const s5_codec *bo, int64_t base)
{
...
int
s5fs_mkfs(S5FS *fs, int fd, uint32_t nblocks, const s5fs_opts *opts)
{
