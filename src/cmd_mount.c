...
static int
s5_getattr(const char *path, struct stat *st, struct fuse_file_info *fi)
{
...
static int
fill_cb(void *arg, uint32_t ino, const char *name)
{
	struct fillc *f = arg;
	(void)ino;
...
static int
s5_readdir(const char *path, void *buf, fuse_fill_dir_t filler,
...
static int
s5_open(const char *path, struct fuse_file_info *fi)
{
	if (!rw_namei(&H, path))
		return -ENOENT;
...
static int
s5_read(const char *path, char *buf, size_t size, off_t off, struct fuse_file_info *fi)
{
	uint32_t ino = rw_namei(&H, path);
	fsr_inode in;
	(void)fi;
...
static int
s5_chmod(const char *path, mode_t mode, struct fuse_file_info *fi)
{
	(void)fi;
	return rw_chmod(&H, path, mode);
}

static int
s5_chown(const char *path, uid_t uid, gid_t gid, struct fuse_file_info *fi)
{
	(void)fi;
...
static int
s5_utimens(const char *path, const struct timespec tv[2], struct fuse_file_info *fi)
{
	(void)fi;
	return rw_utimes(&H, path, (int32_t)tv[0].tv_sec, (int32_t)tv[1].tv_sec);
}

static int
s5_create(const char *path, mode_t mode, struct fuse_file_info *fi)
{
	(void)fi;
	return rw_creat(&H, path, mode, NULL);
}

static int
s5_mkdir(const char *path, mode_t mode)
{
	return rw_mkdir(&H, path, mode);
}

static int
s5_unlink(const char *path)
{
	return rw_unlink(&H, path);
}

static int
s5_rmdir(const char *path)
{
	return rw_rmdir(&H, path);
}

static int
s5_rename(const char *from, const char *to, unsigned int flags)
{
	if (flags)
		return -EINVAL; /* no RENAME_NOREPLACE/EXCHANGE */
	return rw_rename(&H, from, to);
}

static int
s5_write(const char *path, const char *buf, size_t size, off_t off, struct fuse_file_info *fi)
{
	uint32_t ino = rw_namei(&H, path);
	long r;
	(void)fi;
...
static int
s5_truncate(const char *path, off_t len, struct fuse_file_info *fi)
{
	uint32_t ino = rw_namei(&H, path);
	(void)fi;
	if (!ino)
...
cmd_mount(int argc, char **argv)
{
	...
	while ((c = getopt(argc, argv, "B:A:fwd:P:o:")) != -1) {
		...
		case 'A': {
			...
			if (e == S5_NENDIAN) {
				fprintf(stderr, "s5fs mount: bad -A byte order\n");
				return 2;
			}
			forced = (int)e;
			break;
...
{
	if (argc != 2) {
		fprintf(stderr, "usage: s5fs umount mountpoint\n");
		return 2;
	}
