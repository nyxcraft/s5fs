...
static void     le16_put(uint8_t *b, uint16_t v) { b[0] = (uint8_t)v; b[1] = (uint8_t)(v >> 8); }
static uint16_t le16_get(const uint8_t *b) { return (uint16_t)(b[0] | (b[1] << 8)); }

/* ---- big-endian 16-bit ---- */
static void     be16_put(uint8_t *b, uint16_t v) { b[0] = (uint8_t)(v >> 8); b[1] = (uint8_t)v; }
static uint16_t be16_get(const uint8_t *b) { return (uint16_t)((b[0] << 8) | b[1]); }

/* ---- PDP-11 middle-endian 32-bit + PDP l3 3-byte ---- */
static void pdp32_put(uint8_t *b, uint32_t v)
{
	b[0] = (uint8_t)(v >> 16); b[1] = (uint8_t)(v >> 24);	/* high word, LE */
	b[2] = (uint8_t)(v);       b[3] = (uint8_t)(v >> 8);	/* low  word, LE */
}
static uint32_t pdp32_get(const uint8_t *b)
{
	return ((uint32_t)b[1] << 24) | ((uint32_t)b[0] << 16) |
	       ((uint32_t)b[3] << 8)  |  (uint32_t)b[2];
}
static void pdp24_put(uint8_t *b, uint32_t v)
{
	b[0] = (uint8_t)(v >> 16); b[1] = (uint8_t)(v); b[2] = (uint8_t)(v >> 8);
}
static uint32_t pdp24_get(const uint8_t *b)
{
	return ((uint32_t)b[0] << 16) | ((uint32_t)b[2] << 8) | (uint32_t)b[1];
}

/* ---- straight little-endian 32-bit + 3-byte ---- */
static void le32_put(uint8_t *b, uint32_t v)
{
	b[0] = (uint8_t)v; b[1] = (uint8_t)(v >> 8);
	b[2] = (uint8_t)(v >> 16); b[3] = (uint8_t)(v >> 24);
}
static uint32_t le32_get(const uint8_t *b)
{
	return (uint32_t)b[0] | ((uint32_t)b[1] << 8) |
	       ((uint32_t)b[2] << 16) | ((uint32_t)b[3] << 24);
}
static void le24_put(uint8_t *b, uint32_t v)
{
	b[0] = (uint8_t)v; b[1] = (uint8_t)(v >> 8); b[2] = (uint8_t)(v >> 16);
}
static uint32_t le24_get(const uint8_t *b)
{
...
static void be24_put(uint8_t *b, uint32_t v)
{
	b[0] = (uint8_t)(v >> 16); b[1] = (uint8_t)(v >> 8); b[2] = (uint8_t)v;
}
static uint32_t be24_get(const uint8_t *b)
{
