...
must_num(const char *s, const char *what)
{
	...
	if (*s == '\0' || *end != '\0') {
		fprintf(stderr, "s5fs tar: bad %s: %s\n", what, s);
		exit(2);
	}
	return v;
}
...
static int
open_archive(const char *path, char *tmp, size_t tmpsz)
{
	...
	if (tfd < 0) {
		close(fd);
		tmp[0] = '\0';
		fprintf(stderr, "s5fs tar: mkstemp: %s\n", strerror(errno));
		return -1;
	}
...
static int
readblk(int fd, long pos, uint8_t *b)
{
	ssize_t r;
...
parse_tar(int fd, tnode *root)
{
	...
	for (;;) {
		...
		if (rc == 0)
			break;
		if (rc < 0) {
			fprintf(stderr, "s5fs tar: read error at %ld\n", pos);
			return -1;
		}
		if (all_zero(h))
		...
		if (type == 'L') { /* GNU long name in the data */
			...
			if (lseek(fd, off, SEEK_SET) < 0 || read(fd, longname, m) != m)
				return -1;
			longname[m] = '\0';
			haslong = 1;
...
static int
tar_extract(int argc, char **argv)
{
	s5fs_opts opts;
	S5FS fs;
	tnode *root;
	...
	while ((c = getopt(argc, argv, "B:a:d:b:s:t:P:o:")) != -1) {
		...
		case 'd':
			...
			if (!dev) {
				fprintf(stderr, "s5fs tar: unknown device '%s'\n", optarg);
				return 2;
			}
			break;
		default:
	...
	if (dev) {
		if (blocks || sectors) {
			fprintf(stderr, "s5fs tar: give one of -d/-b/-s\n");
			return 2;
		}
		sectors = dev->blocks;
	}
	if (sectors) {
		if (blocks) {
			fprintf(stderr, "s5fs tar: give -b or -s, not both\n");
			return 2;
		}
		blocks = sectors / per;
	}
	...
	if (base != 0 || plen != 0) {
		if (plen == 0) {
			fprintf(stderr, "s5fs tar: partition needs a length (use -d -P, or -o START:LEN)\n");
			return 2;
		}
		opts.base = base;
		blocks = plen / per;
	...
	if (blocks == 0) {
		fprintf(stderr, "s5fs tar: need a size (-d, -b, or -s)\n");
		usage();
	}
...
static void
thl_add(uint32_t ino, const char *path)
{
	if (g_nthl == g_thlcap) {
...
static void
octfield(uint8_t *p, long v, int w) /* w-byte octal + NUL */
{
	char t[16];
...
static void
ustar_hdr(uint8_t *h, const char *name, unsigned mode, int uid, int gid,
	  long size, long mtime, char type, const char *link, int maj, int min_)
{
	...
	if (type == '3' || type == '4') {
		octfield(h + 329, maj, 8);
		octfield(h + 337, min_, 8);
	}
...
static void
wr(int fd, const void *b, size_t n)
{
	if (write(fd, b, n) != (ssize_t)n) {
		perror("s5fs tar: write");
		exit(1);
	}
}
...
static void
tar_emit(FSR *r, int out, uint32_t ino, const char *path, const fsr_inode *in)
{
	uint8_t h[512];
...
static int
walk_cb(void *arg, uint32_t ino, const char *name)
{
	...
	if (!strcmp(name, ".") || !strcmp(name, ".."))
		return 0;
	if (c->prefix[0])
		snprintf(path, sizeof path, "%s/%s", c->prefix, name);
	else
...
tar_create(int argc, char **argv)
{
	...
	while ((c = getopt(argc, argv, "B:A:d:P:o:")) != -1) {
		...
		case 'A': {
			...
			if (e == S5_NENDIAN) {
				fprintf(stderr, "s5fs tar c: bad -A\n");
				return 2;
			}
			forced = (int)e;
			break;
...
cmd_tar(int argc, char **argv)
{
	if (argc >= 2 && !strcmp(argv[1], "x"))
		return tar_extract(argc - 1, argv + 1);
	if (argc >= 2 && !strcmp(argv[1], "c"))
		return tar_create(argc - 1, argv + 1);
	fprintf(stderr,
