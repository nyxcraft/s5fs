...
typedef struct { int fd; long pos; uint32_t rec; const s5_codec *bo; } DR;
...
static unsigned long must_num(const char *s, const char *what)
{
	...
	if (*s == '\0' || *end != '\0') { fprintf(stderr, "s5fs restore: bad %s: %s\n", what, s); exit(2); }
	return v;
}

static int rec_read(DR *dr, uint8_t *buf)
{
	ssize_t r;
	if (lseek(dr->fd, dr->pos, SEEK_SET) < 0) return 0;
	r = read(dr->fd, buf, dr->rec);
	if (r != (ssize_t)dr->rec) return 0;
	dr->pos += dr->rec;
	return 1;
}

static int gethead(DR *dr, uint8_t *hdr)
{
	if (!rec_read(dr, hdr)) return 0;
	return (int)dr->bo->get16(hdr + C_MAGIC) == DMAGIC;
}

static int htype(DR *dr, const uint8_t *hdr) { return (int)dr->bo->get16(hdr + C_TYPE); }
 ...
 */
static int skip_map(DR *dr, uint8_t *hdr)
{
	uint8_t tmp[P11_MAXBSIZE];
	uint16_t n = dr->bo->get16(hdr + C_COUNT);
	while (n--)
		if (!rec_read(dr, tmp)) return 0;
 ...
 */
static int read_blocks(S5FS *fs, DR *dr, uint8_t *hdr, int32_t *da, uint32_t nblk)
{
	...
	for (;;) {
		...
		for (i = 0; i < count && b < nblk; i++, b++) {
			if (ab[C_ADDR + i]) {
				if (!rec_read(dr, rec)) return 0;
				if (fs && da) { da[b] = s5fs_alloc(fs); s5fs_wtblk(fs, (uint32_t)da[b], rec); }
			} else if (da) {
				da[b] = 0;			/* hole */
			}
		}
		if (b >= nblk) {			/* inode satisfied: skip trailing TS_ADDR */
			do { if (!gethead(dr, hdr)) return 0; } while (htype(dr, hdr) == TS_ADDR);
			return 1;
		}
		if (!gethead(dr, hdr)) return 0;
		if (htype(dr, hdr) != TS_ADDR) return 1;	/* next real header */
		memcpy(ab, hdr, dr->rec);
	}
}
...
int cmd_restore(int argc, char **argv)
{
	...
	while ((c = getopt(argc, argv, "B:d:b:s:P:o:")) != -1) {
		...
		case 'd':
			...
			if (!dev) { fprintf(stderr, "s5fs restore: unknown device '%s'\n", optarg); return 2; }
			break;
		default: usage();
		}
	}
	...
	if (base != 0 || plen != 0) {			/* restore into a partition window */
		if (plen == 0) { fprintf(stderr, "s5fs restore: partition needs a length (use -d -P, or -o START:LEN)\n"); return 2; }
		opts.base = base;
		blocks = plen / per;			/* the restored fs fills the partition */
	}
	if (blocks == 0) { fprintf(stderr, "s5fs restore: need a target size (-d, -b, or -s)\n"); usage(); }

	dr.fd = open(dumpfile, O_RDONLY);
	if (dr.fd < 0) { fprintf(stderr, "s5fs restore: %s: %s\n", dumpfile, strerror(errno)); return 1; }
	dr.rec = opts.bsize;
	dr.bo  = s5_codec_for(S5_PDP11);

	/* pass 1: highest inode number (to size the i-list) */
	dr.pos = 0;
	if (!gethead(&dr, hdr)) { fprintf(stderr, "s5fs restore: %s: not a dump (bad magic)\n", dumpfile); close(dr.fd); return 1; }
	for (;;) {
		int t = htype(&dr, hdr);
		if (t == TS_END) break;
		if (t == TS_INODE) {
			...
			if (inum > maxino) maxino = inum;
			if (!read_blocks(NULL, &dr, hdr, NULL, nblk)) break;
		} else if (!skip_map(&dr, hdr)) {
			break;
		}
	}
	if (maxino == 0) { fprintf(stderr, "s5fs restore: no inodes found\n"); close(dr.fd); return 1; }
	...
	if (opts.base != 0 || plen != 0) {
		...
		if (fd < 0) { fprintf(stderr, "s5fs restore: %s: %s\n", image, strerror(errno)); close(dr.fd); return 1; }
		if (fstat(fd, &st) == 0 && st.st_size < need && ftruncate(fd, need) < 0) {
			fprintf(stderr, "s5fs restore: %s: %s\n", image, strerror(errno)); close(fd); close(dr.fd); return 1;
		}
	} else {
	...
	if (s5fs_begin(&fs, fd, (uint32_t)blocks, &opts) < 0) {
		fprintf(stderr, "s5fs restore: %s\n", fs.err); close(fd); close(dr.fd); return 1;
	}
	fs.s_tfree = 0;
	...
	for (;;) {
		...
		if (t == TS_END || fs.error) break;
		if (t != TS_INODE) {
			if (!skip_map(&dr, hdr)) break;
			continue;
		}
...

		{
			...
			free(da);
			nrestored++;
			if (!ok) break;			/* EOF / truncated dump */
		}
	}
	s5fs_finish(&fs);
