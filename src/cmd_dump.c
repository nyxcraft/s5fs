...
static void
wput(const void *b, size_t n)
{
	if (write(OUT, b, n) != (ssize_t)n) {
		perror("s5fs dump: write");
		exit(1);
	}
}

static void
tap_flush(void) /* emit the buffered blocks as one tape record */
{
	uint8_t m[4];
...
static void
indir(uint32_t blk, int level, uint32_t ino)
{
...
static void
dump_inode(uint32_t ino, int *ndir, int *nreg, int *nspec)
{
	...
	for (i = R.laddr; i < R.naddr; i++) {
		uint32_t d = R.bo->get24(dino + P11_DI_ADDR + 3 * i);
		if (d)
			indir(d, (int)(i - R.laddr), ino);
	}
}

/* write a TS_BITS / TS_CLRI record + its map data blocks */
static void
write_map(const uint8_t *map, uint32_t maxino, int typ)
{
...
cmd_dump(int argc, char **argv)
{
	...
	while ((c = getopt(argc, argv, "B:A:d:P:o:T")) != -1) {
		...
		case 'A': {
			...
			if (e == S5_NENDIAN) {
				fprintf(stderr, "s5fs dump: bad -A\n");
				return 2;
			}
			forced = (int)e;
			break;
	...
	if (!map) {
		fprintf(stderr, "s5fs dump: out of memory\n");
		return 1;
	}
	...
	if (maxino == 0) {
		fprintf(stderr, "s5fs dump: no allocated inodes\n");
		return 1;
	}
