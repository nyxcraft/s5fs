...
static uint32_t crc32_upd(uint32_t crc, const uint8_t *p, long n)
{
...
static uint32_t file_crc(FSR *r, const fsr_inode *in)
{
	uint32_t crc = 0;
	long off = 0;
	uint8_t buf[8192];
	while (off < in->size) {
		...
		if (fsr_readfile(r, in, buf, want, off) != want) break;
...
struct wctx { FSR *r; const char *prefix; ent_cb cb; void *arg; };
static int walk_cb(void *a, uint32_t ino, const char *name)
{
	struct wctx *w = a;
	char child[1200];
...
static void walk(FSR *r, uint32_t ino, const char *path, ent_cb cb, void *arg)
{
	fsr_inode in;
	struct ent e;
	if (fsr_iget(r, ino, &in) < 0) return;
	e.mode = in.mode & 07777;
	e.uid = (uint16_t)in.uid; e.gid = (uint16_t)in.gid;
	e.size = in.size; e.mtime = in.mtime; e.path = path;
	switch (in.mode & P11_IFMT) {
	case P11_IFDIR:
		e.type = 'd'; strcpy(e.cks, "-");
		cb(arg, &e);
		{ struct wctx w; w.r = r; w.prefix = path; w.cb = cb; w.arg = arg;
		  fsr_readdir(r, &in, walk_cb, &w); }
		return;
	case P11_IFREG:
		e.type = 'f'; snprintf(e.cks, sizeof e.cks, "%08x", file_crc(r, &in));
		break;
...
static int open_image(int argc, char **argv, FSR *r, const char *usage)
{
...
/* ---- manifest ---- */
static void print_cb(void *arg, const struct ent *e)
{
	FILE *out = arg;
...
/* ---- verify ---- */
struct ment { struct ent e; char *path; int seen; };
struct vctx { struct ment *m; int n; int extra, differ; };

static struct ment *find_m(struct vctx *v, const char *path)
{
	int i;
	for (i = 0; i < v->n; i++)
		if (!strcmp(v->m[i].path, path)) return &v->m[i];
	return NULL;
}

static void verify_cb(void *arg, const struct ent *e)
{
	struct vctx *v = arg;
	struct ment *m = find_m(v, e->path);
	int bad = 0;
	if (!m) { printf("+ %s   (in image, not in manifest)\n", e->path); v->extra++; return; }
	m->seen = 1;
	if (m->e.type != e->type) { printf("! %s: type %c != %c\n", e->path, e->type, m->e.type); bad = 1; }
	if (m->e.mode != e->mode) { printf("! %s: mode %o != %o\n", e->path, e->mode, m->e.mode); bad = 1; }
	if (m->e.uid != e->uid || m->e.gid != e->gid)
		{ printf("! %s: owner %d:%d != %d:%d\n", e->path, e->uid, e->gid, m->e.uid, m->e.gid); bad = 1; }
	if (e->type == 'f') {				/* size + content for files */
		if (m->e.size != e->size) { printf("! %s: size %d != %d\n", e->path, e->size, m->e.size); bad = 1; }
		if (strcmp(m->e.cks, e->cks)) { printf("! %s: content %s != %s\n", e->path, e->cks, m->e.cks); bad = 1; }
	} else if (e->type == 'c' || e->type == 'b') {	/* device numbers */
		if (strcmp(m->e.cks, e->cks)) { printf("! %s: device %s != %s\n", e->path, e->cks, m->e.cks); bad = 1; }
	}
	if (bad) v->differ++;
}
...
int cmd_verify(int argc, char **argv)
{
	...
	if (idx < 0) return 2;
	if (idx + 1 >= argc) { fprintf(stderr, "s5fs verify: need a manifest file\n"); fsr_close(&r); return 2; }
	mf = fopen(argv[idx + 1], "r");
	if (!mf) { fprintf(stderr, "s5fs verify: %s: cannot open\n", argv[idx + 1]); fsr_close(&r); return 1; }

	v.m = malloc((size_t)cap * sizeof *v.m);
	v.n = 0; v.extra = 0; v.differ = 0;
	if (!v.m) { fprintf(stderr, "s5fs verify: out of memory\n"); return 1; }
	while (fgets(line, sizeof line, mf)) {
		char ty[8], cks[24], path[1200]; unsigned long mode; long uid, gid, size, mtime;
		struct ment *e;
		if (line[0] == '#' || line[0] == '\n') continue;
		if (sscanf(line, "%1s %lo %ld %ld %ld %ld %23s %1199[^\n]",
		           ty, &mode, &uid, &gid, &size, &mtime, cks, path) != 8)
			continue;
		if (v.n == cap) { cap *= 2; v.m = realloc(v.m, (size_t)cap * sizeof *v.m);
		                  if (!v.m) { fprintf(stderr, "s5fs verify: out of memory\n"); return 1; } }
		e = &v.m[v.n++];
		e->e.type = ty[0]; e->e.mode = (unsigned)mode; e->e.uid = (int)uid; e->e.gid = (int)gid;
		e->e.size = (int32_t)size; e->e.mtime = (int32_t)mtime;
		snprintf(e->e.cks, sizeof e->e.cks, "%s", cks);
		e->path = strdup(path); e->seen = 0;
		if (!e->path) { fprintf(stderr, "s5fs verify: out of memory\n"); return 1; }
	}
	fclose(mf);
	...
	for (i = 0; i < v.n; i++)
		if (!v.m[i].seen) { printf("- %s   (in manifest, not in image)\n", v.m[i].path); missing++; }
